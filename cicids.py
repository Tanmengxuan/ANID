...
flags.DEFINE_bool("test_multi", False, "Test multiple models")

# Models parameters
flags.DEFINE_bool("atten", False, "Train ANID model")
flags.DEFINE_bool("bilstm", False, "Train bilstm model")
flags.DEFINE_bool("crf", False, "Train crf model")
flags.DEFINE_float("drop", 0.7, "keep_prob for dropout")
flags.DEFINE_integer("num_hidden", 600, "hidden size of model")
flags.DEFINE_integer("num_features", 193, "feature size of input")
flags.DEFINE_integer("MAXLEN", 10, "window")
...
"/home/tmx/cicids_alfonso/normed_new_allreducedselect_final_w10o9_*",
"input path of data") 
...
def main(unused_args):
	...
	if FLAGS.atten:
		#pdb.set_trace()
		model_output_pos = att.pos_encoding(Input_X, keep_prob, FLAGS.num_hidden, FLAGS.MAXLEN)
		model_output, enc_attention_weights, dec_attention_weights = att.apply_attention(Input_X, model_output_pos, FLAGS.num_hidden, FLAGS.MAXLEN, keep_prob)

	elif FLAGS.bilstm:
		model_output = bi.bi_lstm(Input_X, FLAGS.num_hidden, FLAGS.MAXLEN, keep_prob)	
	...
	if FLAGS.train:	
		...
		#	saver.restore(sess,FLAGS.save_path + 'cicids63')
			...
			plot_performance(train_loss_list, val_loss_list, train_f1_list, val_f1_list, FLAGS.model_name)
			h5 = h5py.File('/home/tmx/cisdem/CrossFire-Detect/models/h5files/' + FLAGS.model_name + '_records.h5', 'w')
	...
	if FLAGS.test:
		...
		with tf.Session(config= config) as sess:
			...
			print ('atten:{}, keep_prob:{}, bilstm_hidden:{}, features:{}, MAXLEN:{}, epoch:{}, batch:{}, model:{}, threshold:{}'\
